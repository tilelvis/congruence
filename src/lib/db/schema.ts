...
export const paymentIntents = pgTable('payment_intents', {
  ...
  token: text('token').notNull(), // 'ALN'
  network: text('network').notNull(), // 'alien'
...
export const gameWallets = pgTable('game_wallets', {
  id: uuid('id').primaryKey().defaultRandom(),
  alienId: text('alien_id').notNull().unique(),
  balance: integer('balance').notNull().default(0), // ALN tokens (integer)
...
export const walletLedger = pgTable('wallet_ledger', {
  ...
  txHash: text('tx_hash'), // Alien tx hash
  createdAt: timestamp('created_at').defaultNow().notNull(),
});
